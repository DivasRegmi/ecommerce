...
module.exports = {
  ...
  autoSuggestionFetch: async (req, res) => {
    ...
    var product = await productModel
      ...
      .limit(10)
      .exec();
    var categorySearch = await productModel
      .aggregate([
    ...
    var subCategorySearch = await productModel
      .aggregate([
        {
          $match: {
            subCategory: {
    ...
    output =
      output +
      `<li class="srch-bar-title"> <span class="col-lg-12 search-title">Result</span></li> `;

    product.forEach((pro, i) => {
      output =
        output +
        `<li > <a href="/show/product/${pro._id}">${pro.name} <span> in </span> <span> ${pro.subCategory} </span> </a> </li>`;
    });
    output =
      output +
      `<li class="srch-bar-title"> <span class="col-lg-12 search-title">Related Categories</span></li> `;

    categorySearch.forEach((category, i) => {
      output =
        output +
        `<li> <a href="/product/${category._id.category}">${category._id.category}</a></li> `;
    });

    subCategorySearch.forEach((subCategory, i) => {
      output =
        output +
        `<li> <a href="">${subCategory._id.subCategory} <span> in </span> <span>${subCategory._id.category}</span></a></li> `;
    });
    res.send(output);
  },
      ...
      output =
        ...
        `" alt="">
                        ...
                        <div class="row product-meta-data">
                            ...
                            <div class="col-md-12">
                                <h3 class="sub-heading t compact">` +
        pro.name +
        `</h3>
                            </div>
                            <div class="col-md-12 m-half-auto">
                                <span class="small-para">` +
        pro.subCategory +
        `</span>
                            </div>
